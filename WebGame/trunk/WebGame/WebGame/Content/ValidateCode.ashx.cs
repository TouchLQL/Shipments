...
namespace MultiColorPen.SSIA.Content
{
    /// <summary>
    /// ValidateCode 的摘要说明
    /// </summary>
    public class ValidateCode : IHttpHandler ,IRequiresSessionState   //第二步：实现接口   到此就可以像平时一样用Session了
    {
        ...
        private string MakeCode(int codeLen)
        {
            ...
            SessionHelper.WriteSession(SessionKeys.CheckCodeKey, checkCode);
            return checkCode;
        }
        ///<summary>
        /// 获取验证码图片流
        ...
        private MemoryStream CreateCodeImg(string checkCode)
        {
            ...
            Bitmap image = new Bitmap((int)Math.Ceiling((checkCode.Length * 14.5)), 25);
