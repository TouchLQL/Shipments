...
$(document).ready(function () {
    ...
    myGrid = {
        ...
        columns: [
            ...
            { data: "logtype", title: "LogType" },
            { data: "createtime", title: "CreateTime" },
            { data: "usercode", title: "UserCode" },
            { data: "username", title: "UserName" },
            { data: "logcontent", title: "LogContent" }
        ],
        columnDefs: [{
        ...
        ajax: function (data, callback, settings) {
            //封装请求参数
            var reUrl = '/System/GetLogList';
            var order = data.columns[data.order[0].column].data + " " + data.order[0].dir;
            var pagesize = data.length;
            var pageno = (data.start / data.length) + 1;
            //查询条件
            var s_logtype_val = $('#s_logtype').val();
            var datest = $('#date_st').val();
            var dateed = $('#date_ed').val();
            var searchData = { pagesize: pagesize, pageno: pageno, order: order, logtype: s_logtype_val, datest: datest, dateed: dateed };
            //ajax请求数据
            $.ajax({
                type: "GET",
                url: reUrl,
...
function LogDelete() {
    ...
    $.ajax({
        ...
        url: "/System/LogManageDelete?id=" + seletedItem.id + "&t=" + new Date().getTime(),
        data: "",
